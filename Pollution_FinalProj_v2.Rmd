...
A first visual inspection of the standardized one-step-ahead forecast error series (see Panel (a) in Figure \ref{fig:innovation_process}) shows both a seasonal behavior and a clear change in variance around 2008. We can take into account this second evidence allowing $\sigma_{v}^{2}$ to be time dependent.
...
In Table \ref{tab:MLE_LG} are presented the MLEs of the unknown coefficients for both the time-variant and the time-invariant models and the associated standard errors\footnote{We treat $\psi = (\sigma_{v}^{2}, \; \sigma_{w_1}^{2}, \;\sigma_{w_2}^{2})$ as unknown parameters and we estimate them by maximum likelihood. Then, we perform the analysis of interest plugging them in the model as if they were known costants. We proceed in this way also in the following sections.}.
In Figure \ref{fig:innovation_process}, instead, we compare the standardized innovation process of the two specifications. The second specification clearly represents an improvement, as the innovation process does not show a clear change in variance anymore and looks closer to a random noise\footnote{Note that there is still an uncaptured seasonal behaviour and some other effects: we will try to capture them through a regression component.}. 
...
```{r, fig.width=20, fig.height=3.5, out.width='100%',fig.cap="\\label{fig:innovation_process}Standardized one-step-ahead forecast errors: (a) Time-invariant model; (b) Time-variant model."}
...
\textcolor{red}{ELIMINARE 10-STEP-AHEAD FORECAST. POTREMMO RIEMPIRE METTENDO MODEL CHECKING DI UN PAIO DI MODELLI E CONFRONTARE OPPURE LASCIARE SEMPLICEMENTE QQPLOT E ACF}
...
```{r, fig.width=10, fig.height=4, out.width='50%',fig.cap="\\label{fig:innovation_process_combined}Standardized one-step-ahead forecast errors: Time-variant linear growth model + seasonal DLM."}
...
\end{equation*}
that is, the two time varying coefficients $\alpha_t$ and $\beta_t$ are modeled as independent random walks \textcolor{red}{(we consider both the system variances unknown)}.
...
Let's now compute the filtering estimates of $\theta_t=[\alpha_t \; \beta_t]'$, for $t=0,\ldots,T$. The filtering estimates of $\beta_t$ are plotted over time together with their $90\%$ credible intervals in Figure \ref{fig:filt_est}. \textcolor{red}{SPIEGARE IL PERCHè PLOTTIAMO I FILTERING ESTIMATES QUANDO IN GENERALE SIAMO INTERESSATI A MODELLARE CHANGE POINTS E 1 STEP AHEAD PREDICTION}
...
```{r, fig.width=20, fig.height=3.5, fig.cap="\\label{fig:filt_est} Filtering estimates of alpha (Panel a) and beta (Panel b) with credible intervals"}
#Create df
...
```{r, fig.width=6, fig.height=4, fig.cap="\\label{fig: summary}Time-invariant linear growth model: (a) 10-step-ahead forecast; (b) ACF of one-step-ahead forecast errors; (c) Normal probability plot of standardized one-step-ahead forecast errors."}

source("LG_model_checking.R")

mod1 <- ggarrange(b,c, ncol = 2,labels = c("(a)", "(b)"),font.label=list(size=10, family="Times"))

source("LG_V_model_checking.R")

mod2 <- ggarrange(b,c, ncol = 2,labels = c("(c)", "(d)"),font.label=list(size=10, family="Times"))

source("LG_V_seas_mod_checking.R")

mod3 <- ggarrange(b,c, ncol = 2,labels = c("(e)", "(f)"),font.label=list(size=10, family="Times"))
...
```{r, fig.width=12, fig.height=4, out.width='100%', fig.cap="\\label{fig:one-step-forecast_est}One-step-ahead forecast estimates: (a) Homogeneous HMM; (b) Time-varying linear growth; (c) Time-varying seasonal linear growth; (d) Dynamic regression."}
...
# Plot
colors <- c("Observed data" = "black", "Homogeneous HMM" = "red", "Time-invariant lin.growth" = "firebrick", 
            "Time-varying lin.growth"="blue", "Time-varying seasonal lin.growth"="springgreen4",
            "Dynamic regression"="darkorange")
...
ggarrange(plot_HMM, plot_LG_V, plot_LG_V_seas, plot_DR, ncol=2, nrow=2, align="v",
          labels=c("(a)","(b)","(c)","(d)"), font.label=list(size=9, family="Times"))
```
...
knitr::kable(for_accuracy, "latex", booktabs=T, align="c",
             col.names=c('', 'Time-invar LG', 'Time-var LG', 'Time-var LG + Seas', 'DR'),
...
# Estimate MLE of parameters
fit_SUTSE <- dlmMLE(y_m, parm=rep(0.5, 6), method="L-BFGS-B", lower=c(0.0001,0.0001,0,0,0,0), build_SUTSE, hessian=TRUE)
param_SUTSE <- unlist(build_SUTSE(fit_SUTSE$par)[c("V","W")])
#fit_SUTSE$convergence

# Calculate SE of the MLE using delta method
estVar_SUTSE <- solve(fit_SUTSE$hessian)
...
```{r, fig.height=4, out.width='100%', fig.cap="\\label{fig:SUTSE_forecast}One-step-ahead forecast with credible intervals: (a) CO; (b) SO2."}
