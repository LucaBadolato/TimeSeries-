...
\subsection{Section 2. Univariate DLMs}
\squeeze
...
```{r, fig.width=18, fig.height=3.5, out.width='100%',fig.cap="\\label{fig:innovation_process}Standardized one-step-ahead forecast errors: (a) Time-invariant LG model; (b) Time-variant LG model."}
...
```{r, fig.width=9, fig.height=3.5, out.width='50%', fig.cap="\\label{fig:innovation_process_combined} Standardized one-step-ahead forecast errors: Time-variant seasonal LG model."}
...
\textcolor{red}{In the next section we will follow a different approach that may, appropriately specified, allow for a causal interpretation of results. --> TOGLIEREI CONCETTO DI CAUSALITA', ANCHE SE "APPR. SPECIFIED". SERVE QUESTA FRASE? COSA VOGLIAMO DIRE?}
\squeeze
...
These graphs show the relevance of the break point also in the relationship between $\text{CO}$ and the covariate $\text{SO}_2$. On the one hand, the $\alpha$'s obtained from the filtering procedure, confirming the findings of the HMM section, are decreasing though becoming roughtly constant exactly around the break, namely around 2008. On the other hand, the interpretation of the filtered $\beta$'s in light of the break point is more difficult. However, we observe a mild and unstable relationship between the two variables in the pre-break period (i.e. the coefficient oscillates around zero). It becomes stronger and negative immediatly before the break, whereas becoming positive and increasing during the post-break period. Explanation of this behavior are of a technical kind, being related to the chemical relation between the two pollutants, hence we are not going to dive deeper into a causal explanation. It is enough for us to show that the model is able to track this dynamic relation even in presence of structural instability.
...
From these plots, one can clearly see the effect that the \textit{signal-to-noise ratio} has on the one-step-ahead forecast. \textcolor{red}{For example, the time-invariant linear growth model in reports a relative magnitude of $W$ on $V$ higher than the time-variant one and, as a consequence, its point forecasts tend to follow more closely the data. --> ATTENZIONE! NON ABBIAMO PIU' TIME-INVARIANT: CAMBIARE!}
\squeeze

\subsection{Section 3. Multivariate DLMs}
\smallsqueeze
...
\end{equation*}
We also assume $V_{t}$ to be a time-variant diagonal matrix, similarly as we did above, considering January 2008 as change point. Moreover, to simplify the general model in order to reduce the overall number of parameters, we assume that only the level $\mu_{i,t}$ is correlated among pollutants, which we think is a reasonable assumption in this context.
...
MLEsum_table_2 <- knitr::kable(MLEsum, "latex", booktabs=T, align="c",
  ...
  footnote(general="Standard errors in parentheses.",
           ...
           footnote_as_chunk=T, escape=F) %>%
  kable_styling(latex_options="hold_position")
add_header_above(MLEsum_table_2, c("Months 1 to 84"=2, "Months 85 to 186"=2, " "=5))
```
...
Finally, in Table \ref{tab:for_accuracy} we report the measures of forecasting accuracy for all the models of the analysis. Most interestingly, the table tells us that, in this case, introducing dependence across time series of different pollutants does not "borrow" so much strenght. Indeed, the SUTSE is the best model only according to the MAE, while the MSE and MAPE select less sophisticated specifications. Anyway, such a result should not be taken as absolute, since it could only be due to a poor pollutant choice.
...
To conclude, we have presented several models that can be used to examine the series CO, underlying for each of them advantages and disadvantages. Even if, given the complexity of the phenomenon presented, our specifications may be too basic to have accurate predictions, we can be satisfied with the results we obtained. \textcolor{red}{We have not only learned how to implement the models seen throughout the course, but also studied a debated topic we are interested it, better understanding the evolution of the series across past years. Further research that we are considering are focused on applying the same analysis to other states and pollutants. --> EVITEREI RIFERIMENTI AL CORSO: IL REPORT DEVE ESSERE "UNA COSA CHE PRESENTERESTI AL TUO CAPO" (VEDI SCORECARD). QUI METTEREI COLLEGAMENTO CON LA PURPOSE DEL LAVORO: COSA SAPPIAMO DIRE ORA IN PIU' RISPETTO AL TEMA POLLUTION?}

\textcolor{red}{DA FARE: (1) MIGLIORARE FLUIDITA' DISCORSO (e.g. EVITIAMO A INIZIO SECTION "IN THIS SECTION WE DO..."; (2) RIVEDERE E MAGARI AGGIUNGERE QUALCHE VALORE PER SIGNAL-TO-NOISE RATIO; (3) (SE POSSIBILE) QUALCHE COMMENTO SU MLE CHE TROVIAMO, ALTRIMENTI ABBIAMO TABELLE QUASI PER NIENTE; }
\squeeze
