...
build_DR <- function(param){dlmModReg(x, dV=exp(param[1]), dW=c(exp(param[2]), exp(param[3])), m0=c(y[1],0))}
...
smooth_state_est_alpha <- dropFirst(cO_Smooth$s[,1])
smooth_state_est_beta  <- dropFirst(cO_Smooth$s[,2])

# Smoothing variances
smooth_var <- dlmSvd2var(cO_Smooth$U.S, cO_Smooth$D.S)
smooth_sd <- (unlist(smooth_var))^.5
alpha_sd <- vector(mode = "numeric", length = length(smooth_var))

for (i in 1:length(smooth_var)){ alpha_sd[i] <- smooth_sd[1+i*4] }

smooth_sd_est_alpha <- dropFirst(alpha_sd)
beta_sd <- vector(mode = "numeric", length = length(smooth_var))

for (i in 1:length(smooth_var)){ beta_sd[i] <- smooth_sd[i*4] }

smooth_sd_est_beta <- dropFirst(beta_sd)

# C.I. upper and lower bounds
upper_smooth_alpha <- smooth_state_est_alpha + 1.64*smooth_sd_est_alpha # upper bound of 90% C.I.
lower_smooth_alpha <- smooth_state_est_alpha - 1.64*smooth_sd_est_alpha # lower bound of 90% C.I.
upper_smooth_beta <- smooth_state_est_beta + 1.64*smooth_sd_est_beta # upper bound of 90% C.I.
lower_smooth_beta <- smooth_state_est_beta - 1.64*smooth_sd_est_beta # lower bound of 90% C.I.
```


```{r, fig.width=20, fig.height=3.5, out.width='100%', fig.cap="\\label{fig:smooth_est} Smoothing estimates of alpha (Panel a) and beta (Panel b) with credible intervals"}
# Create df
df_co_ci <- data.frame(louisiana_df$n_months, smooth_state_est_alpha, smooth_state_est_beta, 
                       upper_smooth_alpha, lower_smooth_alpha, upper_smooth_beta, lower_smooth_beta)
colnames(df_co_ci) = c("n_months", "smooth_state_est_alpha", "smooth_state_est_beta",
                       "upper_smooth_alpha","lower_smooth_alpha", "upper_smooth_beta","lower_smooth_beta")

# Plot the smoothing estimates with their credible intervals
smooth_alpha <- ggplot(df_co_ci, aes(x=n_months)) +
  geom_line(aes(y=smooth_state_est_alpha), size=0.4) +
  geom_line(aes(y=upper_smooth_alpha), color="red", linetype="dashed", size=0.2) +
  geom_line(aes(y=lower_smooth_alpha), color="red", linetype="dashed", size=0.2) +
  geom_ribbon(aes(ymin=lower_smooth_alpha, ymax=upper_smooth_alpha), alpha=.1, fill="red") +
  theme_bw(base_size=9) +
...
smooth_beta <- ggplot(df_co_ci, aes(x=n_months)) +
  geom_line(aes(y=smooth_state_est_beta), size=0.4) +
  geom_line(aes(y=upper_smooth_beta), color="red", linetype="dashed", size=0.2) +
  geom_line(aes(y=lower_smooth_beta), color="red", linetype="dashed", size=0.2) +
  geom_ribbon(aes(ymin=lower_smooth_beta, ymax=upper_smooth_beta), alpha=.1, fill="red") +
  theme_bw(base_size=9) +
...
ggarrange(smooth_alpha, smooth_beta, ncol=2, nrow=1, labels=c("(a)", "(b)"),
          font.label=list(size=20, family="Times"))
```
...
```{r, fig.width=12, fig.height=3.5, fig.cap="\\label{fig:one-step-forecast_est} One-step-ahead forecast estimates"}
# Build df
df_forecast <- data.frame(louisiana_df$n_months, y, y_filt_LG, y_filt_LG_v, y_filt_LG_v_seas, y_filt_DR) 
colnames(df_forecast) <- c("n_months", "y", "y_filt_LG", "y_filt_LG_v", "y_filt_LG_v_seas", "y_filt_DR") 

# Plot
colors <- c("Observed data" = "black", "Time-invariant lin.growth" = "springgreen4", 
...
ggarrange(plot_LG, plot_LG_V, plot_LG_V_seas, plot_DR, ncol=2, nrow=2, 
          labels=c("(a)", "(b)","(c)","(d)"),font.label=list(size=10, family="Times"))
...
rename(df_SUTSE, n_months = louisiana_df.n_months)
...
```{r}
ggplot(df_SUTSE, aes(x=louisiana_df.n_months)) +
  geom_line(aes(y=y_m[,1]), size=0.4) +
  geom_line(aes(y=y_filt_SUTSE[,1]), size=0.8, color="red") +
  geom_line(aes(y=SUTSE_ub_CO), color="red", linetype="dotted", size=0.2) +
  geom_line(aes(y=SUTSE_lb_CO), color="red", linetype="dotted", size=0.2) +
