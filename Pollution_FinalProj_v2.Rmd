...
```{r, fig.width=18, fig.height=3.5, out.width='100%', fig.cap="\\label{fig:louisiana_plots}Air pollution in the state of Louisiana"}
...
```{r, fig.width=18, fig.height=5, out.width='100%', fig.cap="\\label{fig:HMM_results}State-dependent means and standard deviations"}
...
In addition to explaining such a shifts, we may want predict them using this homogeneous HMM. For this reason, we produce the one-step-ahead forecast treating the data as streaming ones. Further details on this are reported in the \textbf{Performance evaluation} section.
...
\end{equation*}
Particularly, we decided to impose as starting condition $m_0 = [ y_{1} \;\; 0 ]'$ and $C_0 = \text{diag}(0.001,0.001)$, a relatively large variance to take into account our uncertainty.

In Table \ref{tab:MLE_LG} the MLE's of the unknown coefficients are presented for both the time-varying and the time-invariant models and the associated standard errors\footnote{We treat $\psi = (\sigma_{v}^{2}, \; \sigma_{w_1}^{2}, \;\sigma_{w_2}^{2})$ as unknown parameters and we estimate them by maximum likelihood. Then, we perform the analysis of interest plugging them in the model as if they were known costants. We proceed in this way also in the following sections.}.
...
A first visual inspection to the series of standardized one-step-ahead forecast errors (Panel (a), Figure \ref{fig:innovation_process}) reveals the presence of both a seasonal behavior and a clear change in variance around 2008. We can take into account this second evidence allowing $\sigma_{v}^{2}$ to be time dependent.
To this purpose, we estimated a linear growth model specified as above but considering  $\sigma_{v_t}^{2} = \sigma_{v_1}^{2}$ for observations before 2008 and $\sigma_{v_t}^{2} = \sigma_{v_2}^{2}$ for observations after 2008.

In Figure \ref{fig:innovation_process}, Panel (b), we plot the standardized innovation process of the time-varying specification. A certain improvement can be observed, as the change in the variance is milder and the process looks closer to random noise, still  some \textit{strucure} can be detected in standardized residuals such as a seasonal behaviour. Hence, we will try to capture it through a seasonal model.

```{r, fig.width=18, fig.height=3.5, out.width='100%',fig.cap="\\label{fig:innovation_process}Standardized one-step-ahead forecast errors: (a) Time-invariant model; (b) Time-variant model."}
...
In order to evaluate the improvement that a seasonal component has in our analysis, we combined it with the time varying linear growth model previously specified. Figure \ref{fig:innovation_process_combined} plots the series of innovation process obtained from such a specification. Comparing this plot with the ones reported in Figure \ref{fig:innovation_process}, it is easy to see that the seasonal component is no more present in standardazed residuals. In fact, the series looks like a true noise with almost no structure.

```{r, fig.width=9, fig.height=3.5, out.width='50%',fig.cap="\\label{fig:innovation_process_combined}Standardized one-step-ahead forecast errors: Time-variant linear growth model + seasonal DLM."}
...
In the next section we will follow a different approach that may, appropriately specified, allow for a causal interpretation of results.
...
\end{equation*}
that is, the two time varying coefficients $\alpha_t$ and $\beta_t$ are modeled as independent random walks and we consider both the system variances unknown.
...
These graphs show the relevance of the brake point also in the relationship between $\text{CO}$ and the covariate. First, the intercepts obtained from the filtering procedure, confirming findings in the HMM section, are decreasing though becoming roughtly constant exactly around the break, namely around 2008. On the other hand, he interpretation of the filtered $\beta$'s in light of the break point is more difficult. However, we observe a mild and unstable relationship between the two variables in the pre-break period (i.e. the coefficient oscillates around zero). It becomes stronger and negative immediatly before the break, whereas becoming positive and increasing during the post-brake period. Explanation of this behavior are of a technical kind, being related to the chemical relation betwen the two pollutant, hence we are not going to dive deeper into a causal explanation. It will suffice to us to show that the model is able to track this dynamic relation even in presence of structural instability.
...
```{r, fig.width=18, fig.height=3.5, fig.cap="\\label{fig:filt_est} Filtering estimates with credible intervals: (a) alpha; (b) beta."}
...
```{r, fig.width=18, fig.height=12, out.width='80%', fig.cap="\\label{fig: summary}ACF of one-step-ahead forecast errors and Normal probability plot of standardized one-step-ahead forecast errors: (a) Time-invariant LG model; (b) Time-varying LG model; (c) Time-varying seas LG model."}

source("LG_model_checking.R")
mod1 <- ggarrange(b,c, ncol=2, labels = c("(a)"),font.label=list(size=20, family="Times"))

source("LG_V_model_checking.R")
mod2 <- ggarrange(b,c, ncol=2, labels = c("(b)"),font.label=list(size=20, family="Times"))

source("LG_V_seas_mod_checking.R")
mod3 <- ggarrange(b,c, ncol=2, labels = c("(c)"),font.label=list(size=20, family="Times"))

ggarrange(mod1,mod2,mod3, ncol=1, nrow=3, align="h")
```

In this section we are going to check the assumptions made in our model specifications by testing the properties of the innovation process. Specifically we report here the correlogram (left panels) and the qq-plot of standardazed residuals (right panels).

The left panels  of Figure \ref{fig: summary} report evidence of departure from independence. Indeed, in all the three of them the autocorrelation function is significantly different from zero at some lags. This proves the presence of persistency (especially in the short run), which is not fully captured by our relatively simple models.

The right panels of Figure \ref{fig: summary} report evidence of departure from normality. While residuals for the time-invariant linear growth model (Panel (b)) prove the presence of fat tails in the empirical distribution, residuals of the time-varying model (Panel (d)) seem to be closer to a Gaussian distribution in terms of kurtosis. Still, fat tails are present and some positive skewness appears. Very similar results are also obtained for the seasonal model (Panel (f)).\footnote{For sake of brevity we do not report the model checking for the Dynamic regression. It would not be of significant added value being it very similar to results for the Time-varying seasonal linear growth model. However this exercise can be replicated using the provided R code.} These considerations suggest that a better specification should rely on leptokurtic distributions, such as the \textit{student's t} or the \textit{Generalized Pareto Distribution}. However, we do not conduct a deeper analysis, being it beyond the purpose of the present work.
...
```{r, fig.width=18, fig.height=8, out.width='100%', fig.cap="\\label{fig:one-step-forecast_est}One-step-ahead forecast estimates: (a) Homogeneous HMM; (b) Time-varying linear growth; (c) Time-varying seasonal linear growth; (d) Dynamic regression."}
...
plot_HMM <- ggplot(df_forecast[61:109,], aes(x=n_months)) +
  ...
  theme(legend.position = c(.8, .9), legend.background = element_blank(),
        legend.key = element_blank(), legend.text=element_text(size=15, family="Times"),
...
ggarrange(plot_HMM, plot_LG_V, plot_LG_V_seas, plot_DR, ncol=2, nrow=2, align="h",
          labels=c("(a)","(b)","(c)","(d)"), font.label=list(size=20, family="Times"))
```

It is possible to see the effect that the signal-to-noise ratio has on the one-step-ahead forecast. For example, the time invariant linear growth model reports a relative magnitude of $W$ on $V$ higher than the time variant one and, consequently, its point forecasts tend to follow more closely the data.
...
\end{equation*}
We also assume $V_{t}$ to be a time-variant diagonal matrix, similarly as we did above, considering January 2008 as change point. Moreover, to simplify the general model in order to reduce the overall number of parameters, we assume that only the level $\mu_{i,t}$ is correlated among pollutants, which we think it is a reasonable assumption in this context.  
...
In Table \ref{tab:MLE_SUTSE} are presented the MLEs of the unknown coefficients and the associated standard errors.
```{r}
# MLE
...
MLE_sigmaWbeta11  <- paste0(round(exp(fit_SUTSE$par[5]),4))
MLE_sigmaWbeta22  <- paste0(round(exp(fit_SUTSE$par[7]),4))
...
se_sigmaWbeta11  <- paste0('(', round(SE_SUTSE[5],4), ')')
se_sigmaWbeta22  <- paste0('(', round(SE_SUTSE[7],4), ')')
...
MLEsum_table_2 <- knitr::kable(MLEsum, "latex", booktabs=T, align="c",
  ...
  footnote(general="Standard errors in parentheses.",
           ...
           footnote_as_chunk=T, escape=F) %>%
  kable_styling(latex_options="hold_position")
add_header_above(MLEsum_table_2, c("Period 1 to 84"=2, "Period 85 to 186"=2, " "=5))
```



```{r, fig.width=18, fig.height=4, out.width='100%', fig.cap="\\label{fig:SUTSE_forecast}One-step-ahead forecast with credible intervals: (a) CO; (b) SO2."}
...
CO_forecast <- ggplot(df_SUTSE[61:109,], aes(x=n_months)) +
  ...
  theme_bw(base_size=9) +
  labs(x="Year", y=expression(SO~(ppm)), color="black") +
  scale_y_continuous() +
  scale_x_continuous(breaks=seq(61,109,12),
                     labels=format(seq(as.Date("2006/1/1"),as.Date("2010/12/31"), "1 years"), "%Y")) +
  theme(axis.text.x=element_text(size=20, family="Times", margin=margin(t=5)),
...
We have presented several models that can be used to examine the series CO, underlying for each of them advantages and disadvantages. Even if, given the complexity of the phenomenon presented, our specifications may be too basic to have accurate predictions, we are satisfied with the results we obtained. We have not only learned how to implement the models seen throughout the course, but also studied a debated topic we are interested it, better understanding the evolution of the series across past years. Further research that we are considering are focused on applying the same analysis to other States and pollutants. 
