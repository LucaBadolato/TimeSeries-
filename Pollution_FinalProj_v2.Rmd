...
The first DLM specification we consider to examine $\text{CO}$ is a linear growth model, which is suitable to capture both a latent level, $\mu_{t}$, and a time varying splope in its dynamics, $\beta_{t}$. As usual, it is defined as following: 
\begin{align*} 
\begin{cases}
...
x <- as.numeric(coal_cons$combined_heat.power_industrial_power)
...
build_DR <- function(param){dlmModReg(x, dV=exp(param[1]), dW=exp(param[2:3]), m0=c(y[1],0))}
...
# Estimate MLE of parameters
fit_SUR <- dlmMLE(y_m, parm=rep(0.5,10), build_SUR, hessian=TRUE)
