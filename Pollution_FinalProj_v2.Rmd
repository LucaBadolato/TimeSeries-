...
build_DR <- function(param){dlmModReg(x, addInt=TRUE, dV=param[1], dW=param[2:3], m0=c(y[1],0))}

# Estimate MLE of parameters
fit_DR <- dlmMLE(y, parm=rep(1,3), build_DR, hessian=TRUE)
...
buildSUR <- function(param){
  Bmod=dlmModReg(Xm, addInt=TRUE)
  Bmod$FF=matrix(c(1,1), nrow=1, ncol=2) %x% diag(2)
  Bmod$JFF=matrix(c(0,0,0,0,1,0,0,2), nrow=2, ncol=4)
  Bmod$GG=diag(4)
  Bmod$V=bdiag(exp(x[1]), exp(x[2]))
  
  Wb=matrix(c(exp(x[3]), 0, 0, exp(x[4])), nrow=2, ncol=2)
  W=bdiag(0, 0, Wb) # bdiag(exp(x[6]), exp(x[7]), Wb)
  Bmod$W=W  
...
# Estimate MLE of parameters
fit_SUR <- dlmMLE(Ym, parm=rep(1,6), build_SUR, hessian=TRUE)
