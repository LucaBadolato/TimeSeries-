...
\textcolor{red}{Table \ref{tab:HMM_MLE} reports, however, unexpected results. Indeed, the slope estimate in the first state, $\hat{\beta_1}_{MLE}$, is only slightly negative and very close to zero, whereas the estimate for the second state, $\hat{\beta_2}_{MLE}$ is null as we expected. Overall, the model suggestS that the trend in the two states is not different (i.e. it is equal to zero). --> LA STIMA DEL COEFFICIENTE E' TUTT'ALTRO CHE INSIGNIFICATIVA! OLTRE AD ESSERE STATISTICAMENTE SIGNIFICATIVA, E' MOLTO RILEVANTE SE (1) RAPPORTI QUEL VALORE AI VALORI DELLA Y (-0.004/0.5=-0.008, OVVERO -0.8% AL MESE: TANTISSIMO!) E (2) CONSIDERI CHE QUELLA DIMINUZIONE AVVIENE -COME DETTO- A LIVELLO MENSILE.}
...
HMM_main <- ggplot(df_to_plot, aes(x=xtime, y=yvalue)) +
  ...
  scale_y_continuous(breaks=seq(0,1.5,0.25)) +
   theme(plot.margin=unit(c(0.5,1,0.5,0.5), "cm"),
         ...
         axis.title.x=element_text(size=30, family="Times", margin=margin(t=8)),
         axis.title.y=element_text(size=30, family="Times", margin=margin(r=8)))
...
HMM_additional <- ggplot(HMM_additional_CO, aes(time_index, value)) +
  ...
  theme(panel.spacing=unit(1.5, "lines"),
        ...
        strip.text=element_text(size=30, family="Times",face="bold"),
...
# Option 2
ggarrange(HMM_main, HMM_additional, widths=c(3,2))
```
...
A deeper anlysis on the factors explaining this shift can be found in @Feng2015, in which they analyse the series of $\text{CO}_2$.

MODEL CHECKING FOR HMM: DECIDERE L'ORIDINE. LASCIARLO QUI O PRESENTARE ENTRAMBI I MODEL CHECK PER HMM A LGM INSIEME PER CONFRONTARLI MEGLIO? 
```{r, include=FALSE}
#MODEL CHECKING:
#Calculation of fitted values
louisiana_df$state = estStates[1]
louisiana_df$mu = NA
...
```{r echo=FALSE}
# Monthly seasonality based on MLE for the observation equation error in Linear growth model
Season <- dlmModSeas(frequency = 12, dV=exp(fit_LG$par[1]))
```
...
# Df
df_LG_filt <- data.frame(louisiana_df$n_months, y, y_filt)
colnames(df_LG_filt ) = c("n_months", "y", "y_filt")

# Plot
colors <- c("Observed data" = "black", "Filtering estimates" = "red")
ggplot(df_LG_filt, aes(x=n_months)) +
...
LG_seas <- LG + Season
# Kalman filter
LG_seas_Filt <- dlmFilter(y, LG)
# Remove the first item (m_0) in the vector of filtered states
filt_est_seas = dropFirst(LG_Filt$m)
# Compute the vector of state variances and standard deviations
# list_c <- dlmSvd2var(LG_Filt$U.C, LG_Filt$D.C)
# vol_C <- sqrt(unlist(list_c))
# One step ahead forecast
y_filt_seas <- LG_seas_Filt$f
```

```{r, fig.width=7, fig.height=2, fig.cap="\\label{fig:filter_est} Filtering states estimates"}

# Df
df_LG_filt_seas <- data.frame(louisiana_df$n_months, y, y_filt_seas)
colnames(df_LG_filt_seas ) = c("n_months", "y", "y_filt")

# Plot
colors <- c("Observed data" = "black", "Filtering estimates" = "red")
ggplot(df_LG_filt_seas, aes(x=n_months)) +
  geom_line(aes(y=y, color="Observed data"), size=0.4) +
  geom_line(aes(y=y_filt, color = "Filtering estimates"), size=0.4) +
  #geom_vline(xintercept = as.Date("2008/1/1"), color= "black", size=0.3) +
  theme_classic(base_size=9.5) +
...
\textcolor{red}{IL MODEL CHECKING LO LASCEREI SOLO PER HMM E DLM, DIREI SOLTANTO "WE HAVE CHECKED THE RESIDUALS AND THEY..."}
...
estVarLog <- solve(fit_DR$hessian)
estVar_DR <- diag(exp(fit_DR$par)) %*% estVarLog %*% + diag(exp(fit_DR$par))
...
\section{Part 2. Multivariate DLM}
...
\underset{4 \times 1}{\theta_t} = \begin{bmatrix} \\ \alpha_{CO,t} \\ \alpha_{SO_2,t}, \\ \beta_{CO,t} \\ \beta_{SO_2,t} \end{bmatrix}, \quad
...
buildSUR <- function(param){
  ...
  Bmod$JFF=matrix(c(0,1), nrow=1, ncol=2) %x% diag(2)
  Bmod$GG=diag(4)
  Bmod$V=diag(c(exp(param[1]), exp(param[2])))
  Wa=matrix(c(exp(param[3]),param[4],exp(param[5]),param[6]),2,2)
  Wb=matrix(c(exp(param[7]), param[8], exp(param[9]), param[10]),2,2)
  W=bdiag(Wa, Wb) 
  Bmod$W=W
  Bmod$m0=rep(1,2*2)
  Bmod$C0=diag(2*2)
...
# Estimate MLE of parameters
fit_SUR <- dlmMLE(y_m, parm=rep(1,10), buildSUR, hessian=TRUE)
unlist(buildSUR(fit_SUR$par)[c("V","W")])
...
Let us still consider the time series for $\text{CO}$ and $\text{SO}_2$. We now model each time series using a linear growth model. We assume that growth $\beta_{i,t}$ is correlated among pollutants. In order to achieve "borrowing of strength", we put together the two individual DLM using a \textit{Seemingly unrelated time series equations model} (SUTSE):
\begin{align*}
...
\begin{equation*}
\underset{2 \times 1}{Y_t} = \begin{bmatrix} Y_{CO,t} \\ Y_{SO_2,t} \end{bmatrix}, \quad
\underset{4 \times 1}{\theta_t} = \begin{bmatrix} \\ \mu_{CO,t} \\ \mu_{SO_2,t}, \\ \beta_{CO,t} \\ \beta_{SO_2,t} \end{bmatrix}, \quad
\underset{1 \times 2}{F} = \begin{bmatrix} 1 & 0 \end{bmatrix}, \quad
\underset{2 \times 2}{G} = \begin{bmatrix} 1 & 1 \\ 0 & 1 \end{bmatrix} \quad \text{ and } \quad \underset{4 \times 4}{W} = blockdiag(W_{\mu}, W_{\beta}).
\end{equation*}

\textcolor{red}{IL CODICE SEGUENTE E' DA AGGIUSTARE!!! PER ORA SOLO COPIA-INCOLLA DA SUR}

```{r}
# Build SUTSE model
buildSUTSE <- function(param){
  Bmod=dlmModPoly(order=2, addInt=TRUE)
  #Bmod$FF=matrix(c(1,1), nrow=1, ncol=2) %x% diag(2)
  #Bmod$JFF=matrix(c(0,1), nrow=1, ncol=2) %x% diag(2)
  #Bmod$GG=diag(4)
  #Bmod$V=diag(c(exp(param[1]), exp(param[2])))
...
# Estimate MLE of parameters
fit_SUTSE <- dlmMLE(y_m, parm=rep(1,10), buildSUTSE, hessian=TRUE)
unlist(buildSUTSE(fit_SUTSE$par)[c("V","W")])
...
# Put MLE in the model
SUTSE <- buildSUTSE(fit_SUTSE$par)
