...
O3 <- ggplot(louisiana_df, aes(x=n_months, y=m_O3_mean)) +
  ...
  theme(axis.text.x=element_text(size=25, family="Times", margin=margin(t=5)),
        axis.text.y=element_text(size=25, family="Times", margin=margin(r=5)),
        axis.title.x=element_text(size=25, family="Times", margin=margin(t=5)),
        axis.title.y=element_text(size=25, family="Times", margin=margin(r=5)),
...
HMM_main <- ggplot(df_to_plot, aes(x=xtime, y=yvalue)) +
  ...
  scale_y_continuous(breaks=seq(0,1.5,0.25)) +
   theme(axis.text.x=element_text(size=25, family="Times", margin=margin(t=8)),
         axis.text.y=element_text(size=25, family="Times", margin=margin(r=8)),
...
HMM_additional <- ggplot(HMM_additional_CO, aes(time_index, value)) +
  ...
  theme(panel.spacing=unit(1.5, "lines"),
        axis.text.x=element_text(size=25, family="Times", margin=margin(t=8)),
        axis.text.y=element_text(size=25, family="Times", margin=margin(r=8, l=8)),
        axis.title.x=element_text(size=25, family="Times", margin=margin(t=8)),
        axis.title.y=element_blank(),
        panel.grid.major=element_line(size=0.5),
        panel.grid.minor=element_blank(),
        strip.text=element_text(size=25, family="Times",face="bold"),
...
\textcolor{red}{DUBBIO RESIDUI: PENSAVO DI INSERIRE QUI IL CORRELOGRAMMA E IL QQPLOT, COMMENTANDO CHE PER GLI ALTRI MODELLI NON LI AVREMMO RIPORTATI E COSì VIA E MI è VENUTO UN DUBBIO PERCHè VOLEVO STANDARDIZZARLI. COME LI STANDARDIZZO ESSENDOCI TRE VARIANZE? BISOGNA TESTARE FORSE LE TRE EQUAZIONI UNA PER VOLTA IN BASE ALLO STATO? NEL CASO PRENDO IL PRIMO STATO E PLOTTO I DUE GRAFICI COMMENTANTO IL TUTTO.}
...
\begin{equation*}
  ...
  W = \begin{bmatrix} \sigma_{w_1}^{2} & 0 \\ 0 & \sigma_{w_1}^{2} \end{bmatrix}, \quad
  \theta_0 \sim N_{2}(m_0 = \begin{pmatrix} \hat{\mu_0} \\ \hat{\beta_0} \end{pmatrix}, C_0) \text{ and }
  \theta_{0} \indep (v_t) \indep (w_t).
\end{equation*}

\textcolor{red}{STARTING PRIORS TO BE COMMENTED.}
...
For this purpose, we estimated a linear growth model specified as above but considering  $\sigma_{v,t}^{2} = \sigma_{v,1}^{2}$ for observations before 2008 and  $\sigma_{v,t}^{2} = \sigma_{v,2}^{2}$ for observations after 2008. The MLE estimators are reported in Table \ref{tab:MLE_lgm_v}.
...
# Remove the first item (m_0) in the vector of filtered states
filt_est_LG_v = dropFirst(LG_Filt_v$m)

# Compute the vector of state variances and standard deviations
list_c_LG_v <- dlmSvd2var(LG_Filt_v$U.C, LG_Filt_v$D.C)
vol_C_LG_v <- sqrt(unlist(list_c_LG_v))

# One step ahead forecast
y_filt_LG_v <- LG_Filt_v$f
```
...
ggarrange(A, B, ncol=2, nrow=1, align="h",
          labels=c("(a)", "(b)"), font.label=list(size=30, family="Times"))
```
...
```{r}
# MLE
MLE_sigmaV  <- paste0(round(exp(fit_seas$par[1]),5))
MLE_sigmaW <- paste0(round(exp(fit_seas$par[2]),9))

# se(MLE)
se_sigmaV  <- paste0('(', round(SE_seas[1],5), ')')
se_sigmaW <- paste0('(', round(SE_seas[2],9), ')')
...
where
\begin{eqnarray*}
  \theta_t=[\alpha_t \; \beta_t]', \qquad \theta_0 \sim \mathcal{N}_2(m_0, C_0), \qquad \theta_0 \indep (v_t)
  \indep (w_t)
\end{eqnarray*}
and $G=I_2$ and $W=diag(\sigma_{W_{1}}^{2}, \sigma_{W_{2}}^{2})$, i.e the two time varying coefficients $\alpha_t$ and $\beta_t$ are modeled as independent random walks. 
...
```{r, fig.width=6, fig.height=2, fig.cap="\\label{fig:smooth_est} Smoothing estimates of beta with credible intervals"}
# Smoothing estimates
...
Let's plot the filtering states estimates obtained with the models fitted so far (Figure \ref{fig:filter_est}).

```{r, fig.width=7, fig.height=2, fig.cap="\\label{fig:filter_est} Filtering states estimates"}
# Build df
df_filt <- data.frame(louisiana_df$n_months, y, y_filt_LG, y_filt_LG_v, y_filt_LG_v_seas, y_filt_DR) # + struct_fit,
colnames(df_filt) <- c("n_months", "y", "y_filt_LG", "y_filt_LG_v", "y_filt_LG_v_seas", "y_filt_DR") # + "struct_fit",
...
ggplot(df_filt, aes(x=n_months)) +
  ...
  theme(axis.text.x=element_text(size=15, family="Times", margin=margin(t=5)),
        ...
        theme(legend.justification=c("left", "top"), legend.text=element_text(size=12, family="Times"),
              legend.box.just="bottom", legend.margin=margin(2, 2, 2, 2))
...
\begin{eqnarray*}
  ...
  \theta_t = (G \otimes I_2) \; \theta_{t-1} + w_t, & w_t \overset{iid}\sim \mathcal{N}_4(0,W)
\end{eqnarray*}
with
\begin{equation*}
...
\subsection{(ADDITIONAL) 3.2. Seemingly unrelated time series equations (SUTSE)}
...
for_accuracy <- data.frame(#HMM      = c(MSE_HMM,        MAE_HMM,        MAPE_HMM),
...
knitr::kable(for_accuracy, "latex", booktabs=T, align="c",
             col.names=c('HMM', 'Time-invar LG', 'Time-var LG', 'Time-var LG + Seas', 'DR', 'SUR', 'SUTSE'),
             row.names=c('MSE','MAE','MAPE'),
