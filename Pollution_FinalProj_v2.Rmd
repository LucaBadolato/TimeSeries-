...
build_lin_growth <- function(param){dlmModPoly(order=2, dV=exp(param[1]), dW=exp(param[2:3]),m0=c(y[1],0))}

# Estimate MLE of parameters
fit_CO_lin_growth <- dlmMLE(y, rep(1,3), build_lin_growth, hessian=TRUE)
unlist(build_lin_growth(fit_CO_lin_growth$par)[c("V","W")])            # Convergence achieved
...
# Alternative (2): integrated random walk model
  ...
  fit_3 <- dlmMLE(y, rep(1,2), buildMod_2, hessian=TRUE)            # Convergence achieved
...
```{r}
lin_growth <- build_lin_growth(fit_CO_lin_growth$par)
# Kalman filter
lin_growth_Filt <- dlmFilter(y, lin_growth)
# Remove the first item (m_0) in the vector of filtered states
filt_est=dropFirst(lin_growth_Filt$m)
# Compute the vector of state variances and standard deviations
list_c <- dlmSvd2var(lin_growth_Filt$U.C, lin_growth_Filt$D.C)
vol_C <- sqrt(unlist(list_c))
# One step ahead forecast
y_filt <- lin_growth_Filt$f
```

textcolor{red}{NON SONO RIUSCITO A USARE LE STESSE DEGLI ALTRI GRAFICI DATE NELL' ASSE X. QUALCUNO RIESCE A DARCI UN'OCCHIATA??? BISOGNA ANCHE CAMBIARE IN NOME DELL'ASSE y}
```{r, fig.width=7, fig.height=2, fig.cap="\\label{fig:filter_est} Filtering states estimates"}

n_dates <- seq(as.Date("2000/01/01"), as.Date("2016/6/1"), "month")

# Df
df_lin_growth_filt <-data.frame(n_dates, y, y_filt)

# Plot
colors <- c("Observed data" = "black", "Filtering estimates" = "red")
ggplot(df_lin_growth_filt, aes(x=n_dates)) +
  geom_line(aes(y=y, color="Observed data"), size=0.4) +
  geom_line(aes(y=y_filt, color = "Filtering estimates"), size=0.4) +
  geom_vline(xintercept = as.Date("2008/01/01"), color= "black", size=0.3) +
...
textcolor{red}{NEL CASO NON CI FOSSE ABBASTANZA SPAZIO POTREMMO TOGLIERE IL PLOT DEI RESIDUALS CHE DEI 3 MI SEMBRA IL MENO INFORMATIVO}
...
lin_growth_res <- residuals(lin_growth_Filt, sd=FALSE)
lin_growth_res = data.frame(lin_growth_res, n_dates)
colnames(lin_growth_res) = c("res", "time")

a <- ggplot(lin_growth_res, aes(x=time)) +
...
b <- ggplot.corr(data=lin_growth_res$res, lag.max=24, ci=0.95, large.sample.size=FALSE, horizontal=TRUE)

c <- ggplot(lin_growth_res, aes(sample=res)) +
  stat_qq(col="blue") +
...
lin_growth_MSE <- round(mean((lin_growth_Filt$f-y)^2),3) # mean square error (MSE)
lin_growth_MAE <- round(mean(abs(lin_growth_Filt$f-y)),3) # mean abs. error (MAE)
lin_growth_MAPE <- round(mean(abs(lin_growth_Filt$f-y)/y),3) # mean abs. percentage error (MAPE)
```
