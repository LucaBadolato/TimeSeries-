...
A first visual inspection of  series of standardized one-step-ahead forecast errors (Panel (a), Figure \ref{fig:innovation_process}) shows both a seasonal behavior and a clear change in variance around 2008. We can take into account this second evidence allowing $\sigma_{v}^{2}$ to be time dependent.
...
Furthermore, it is possible to check model assumptions testing the properties of the innovation process e.g. through a correlogram (left panels) and a qq-plot (right panels).
...
CO_forecast <- ggplot(df_SUTSE[61:109,], aes(x=louisiana_df.n_months)) +
...
SO2_forecast <- ggplot(df_SUTSE[61:109,], aes(x=louisiana_df.n_months)) +
