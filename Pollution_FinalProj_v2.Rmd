...
```{r, fig.width=20, fig.height=4, out.width='100%', fig.cap="\\label{fig:smooth_est} Smoothing estimates of alpha (Panel a) and beta (Panel b) with credible intervals"}
# Create df
...
# Plot
colors <- c("Observed data (dashed)" = "black", "Time-invariant linear growth" = "springgreen4", 
            "Time-variant linear growth"="blue", "Time-variant linear growth + Seasonality"="firebrick","Dynamic regression"="darkorange")
ggplot(df_forecast[61:109,], aes(x=n_months)) +
  geom_line(aes(y=y, color="Observed data (dashed)"), size=0.4, linetype="longdash") +
...
# Estimate MLE of parameters
fit_SUTSE <- dlmMLE(y_m, parm=rep(1, 6), lower=c(0.0001, 0.0001,0,0,0,0), build_SUTSE, hessian=TRUE)
...
SO2_forecast <- ggplot(df_SUTSE, aes(x=n_months)) +
