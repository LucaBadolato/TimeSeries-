...
MSE_HMM <- round(mean((HMM_forecast-y)^2),3) # mean square error (MSE)
MAE_HMM <- round(mean(abs(HMM_forecast-y)),3) # mean abs. error (MAE)
MAPE_HMM <- round(mean(abs(HMM_forecast-y)/y),3) # mean abs. percentage error (MAPE)
...
MSE_SUTSE <- round(mean((SUTSE_Filt$f[,1]-y)^2),3) # mean square error (MSE)
MAE_SUTSE <- round(mean(abs(SUTSE_Filt$f[,1]-y)),3) # mean abs. error (MAE)
MAPE_SUTSE <- round(mean(abs(SUTSE_Filt$f[,1]-y)/y),3) # mean abs. percentage error (MAPE)