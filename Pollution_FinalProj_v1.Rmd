...
CO_lin_growth <- function(param){dlmModPoly(order=2, dV=param[1], dW=param[2:3])}

# Estimate MLE of parameters
fit_CO_lin_growth <- dlmMLE(y, rep(1,3), CO_lin_growth, lower=c(0.0000001, 0, 0), hessian=TRUE)
fit_CO_lin_growth$convergence # 0 indicates successful convergence: we don't have convergence!!!
unlist(CO_lin_growth(fit_CO_lin_growth$par)[c("V","W")])
fit_CO_lin_growth$hessian

# Alternative (1): assume constant nominal speed in the dynamics
  buildMod_2 <- function(param){dlmModPoly(order=2, dV=param[1], dW=c(param[2], 0))}

  fit_2 <- dlmMLE(y, rep(1,3), buildMod_2, lower=c(0.0000001, 0, 0), hessian=TRUE)
  fit_2$convergence # 0 indicates successful convergence: we don't have convergence!!!
  unlist(buildMod_2(fit_2$par)[c("V","W")])
  fit_2$hessian

# Alternative (2): integrated random walk model
  buildMod_3 <- function(param){dlmModPoly(order=2, dV=param[1], dW=c(0, param[2]))}

  fit_3 <- dlmMLE(y, rep(1,3), buildMod_2, lower=c(0.0000001, 0, 0), hessian=TRUE)
  fit_3$convergence # 0 indicates successful convergence: we don't have convergence!!!
