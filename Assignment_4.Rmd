...
In our analysis we will focus on the four times series related to Alaska, recorded daily from *anno di inizio* to *anno fine*, for a total of 493 observations. The main advantage is given by study the evolution of air pollution in the same country over the years. This allows to control for fixed effects and to better study the latent process of interest that relate the four pollution components. We report in Table \ref{tab:alaska_df} the first five observations of the dataset.

```{r echo=FALSE}
# Import the dataset
alaska_df <- read.csv(url("https://raw.githubusercontent.com/SimoneArrigoni/US_pollution/master//Alaska_dataset.csv"), sep = ",", header=T)

obs <- c(1:493)
alaska_df <- cbind(obs, alaska_df)
```


```{r}
# How the dataset looks like
knitr::kable(alaska_df[1:5, 1:5], "latex", booktabs=T,
             caption="Dataset structure\\label{tab:alaska_df}") %>%
  kable_styling(latex_options = "hold_position")
```

As explained before, the four variables are related by a common latent process. Hence, we report in table \ref{tab:corr} their correlation coefficients. Particularly, we expect to see a positive correlation (**IN REALTà SONO TUTTE MOLTO POSITIVE TRANNE CON 03, CHIEDO MAGARI AL MIO AMICO UNA SPIEGAZIONE**).


```{r}
# Correlation plot
datacor = subset(alaska_df,select=c(daily_O3_mean, daily_NO2_mean, daily_CO_mean, daily_SO2_mean))
...
# Time series plot
O3 <- ggplot(alaska_df, aes(x=obs, y=daily_O3_mean)) +
  geom_line(color="steelblue") +
  theme_classic(base_size=9) +
  labs(y="Ground level ozone (O3)", x="Observation") +
  scale_x_continuous(breaks=seq(0,500,50)) +
  theme(axis.title.x=element_text(family="Times", margin=margin(t=5)),
        axis.title.y=element_text(family="Times", margin=margin(r=5)))

NO2 <- ggplot(alaska_df, aes(x=obs, y=daily_NO2_mean)) +
  geom_line(color="steelblue") +
  theme_classic(base_size=9) +
  labs(y="Nitrogen dioxide (NO2)", x="Observaion") +
  scale_x_continuous(breaks=seq(0,500,50)) +
  theme(axis.title.x=element_text(family="Times", margin=margin(t=5)),
        axis.title.y=element_text(family="Times", margin=margin(r=5)))

CO <- ggplot(alaska_df, aes(x=obs, y=daily_CO_mean)) +
  geom_line(color="steelblue") +
  theme_classic(base_size=9) +
  labs(y="carbon monoxide (CO)", x="observation") +
  scale_x_continuous(breaks=seq(0,500,50)) +
  theme(axis.title.x=element_text(family="Times", margin=margin(t=5)),
        axis.title.y=element_text(family="Times", margin=margin(r=5)))

SO2 <- ggplot(alaska_df, aes(x=obs, y=daily_SO2_mean)) +
  geom_line(color="steelblue") +
  theme_classic(base_size=9) +
  labs(y="Sulfur dioxide (SO2)", x="observation") +
  scale_x_continuous(breaks=seq(0,500,50)) +
  theme(axis.title.x=element_text(family="Times", margin=margin(t=5)),
        axis.title.y=element_text(family="Times", margin=margin(r=5)))
...
We restrict our attention to carbon monoxide (\texttt{CO}) data. It is possible to see that the time series does not look stationary and that several change points characterize the plot, e.g. at observation 120 and 320. Therefore, it is not possible to analyse this series with the well-known ARMA models, which require stationarity (or non-stationary time series that can be easily trasformed into stationary ones). A suitable class of models for these cases is given by the Hidden Markov Models.
