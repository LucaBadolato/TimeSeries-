...
Given that elevation gains/losses are more informative than elevation itself, we create the variable \texttt{dfelev} by first differencing \texttt{elev}.
...
The three variables are related by a common physical process, the bike ride. In table \ref{tab:corr} we reported their correlation coefficients. Particularly, we have:

\begin{itemize}
\item Elevation gains are associated with an increase in the heart rate and a decrease of speed
\item Speed and heart rate have a negative correlation. Apparentely this is counterintuitive, but the real effect is confounded by the fact that speed increases a lot during the downhills where heart rate gets lower. 
\end{itemize}
...
```{r}
# Correlation plot
datacor = subset(tracks_df,select=c(bpm, speed, dfelev))
cor.datacor = cor(datacor, use="complete.obs")
cormat = round(cor(datacor),2)
row.names(cormat) <- c("$\\texttt{bpm}$", "$\\texttt{speed}$", "$\\texttt{dfelev}$")

knitr::kable(cormat, "latex", booktabs=T, align="c",
             escape=F, caption="Correlation coefficients of heart rate, speed and elevation gains/losses\\label{tab:corr}",
             col.names = c("$\\texttt{bpm}$", "$\\texttt{speed}$", "$\\texttt{dfelev}$")) %>%
       column_spec(1:3, width="5em") %>%
kable_styling(latex_options="hold_position")
...
# Plot the optimal path, that is stored in the first column of estStates
# Plot the optimal path, that is stored in the first column of estStates
results_df_bpm <- data.frame(time_index=track_38$time,
                         bpm =track_38$bpm,
                         state=estStates_bpm[1]) %>% 
  gather("variable", "value", -time_index)

my_breaks <- function(x) { if (max(x)<4) seq(0,3,1) else seq(100,170,10) }
lab_names <- c(
  'bpm'="Heart rate (bpm)", 
  'state'="Viterbi state")

plotobj_bpm <- ggplot(results_df_bpm, aes(time_index, value)) +
  geom_line(color="black") +
  facet_wrap(variable~., scales="free", ncol=1,
             strip.position="left",
             labeller = as_labeller(lab_names)) +
