...
se_mu1hat_nh <- paste0('(', round(MLEse_nh$se[13],3), ')')
se_mu2hat_nh <- paste0('(', round(MLEse_nh$se[15],3), ')')
se_mu3hat_nh <- paste0('(', round(MLEse_nh$se[17],3), ')')
se_sd1hat_nh <- paste0('(', round(MLEse_nh$se[14],3), ')')
se_sd2hat_nh <- paste0('(', round(MLEse_nh$se[16],3), ')')
se_sd3hat_nh <- paste0('(', round(MLEse_nh$se[18],3), ')')
...
```{r, out.width='60%', fig.width=6.5, fig.height=3}

#Plot the optimal path, that is stored in the first column of estStates_nh
...
F2_nh<-ggplot(data.frame(switching_mean_nh, data_a4[1],data_a4[6]))+
      geom_line(aes(x=Time, y=ITAbond_real))+
...
ggarrange(F1_nh, F2_nh, labels = c("A", "B"), ncol = 2, nrow = 1)
