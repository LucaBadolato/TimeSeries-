...
```{r echo=FALSE}
# Import the dataset
tracks_df <- read.csv(url("https://raw.githubusercontent.com/LucaBadolato/TimeSeries-/master/tracking_data_start=30min_record=120min_freq=30sec.csv"), sep = "\t", header=T)
```

In this exercise we consider the \texttt{Activity tracking} data set, which containts 45 activities recorded during a bike ride. The variables of interest are the following:
  \begin{itemize}
\item average speed (\textit{speed})
\item heart rate (\textit{bpm})
\item elevation (\textit{elev}),
\end{itemize}
measured at regular 30-second intervals, for a total duration of 2 hours. 

Given that elevation gains/losses are more informative than elevation itself, we create the variable \textit{dfelev} by first differenting \textit{elev}. We report in table \ref{tab:tracks} the first five observations:   
```{r echo=FALSE}
...
```{r}
# How the dataset looks like
knitr::kable(tracks_df[1:5, c(1:5, 8)], "pandoc", caption="Dataset structure\\label{tab:tracks}") %>%
  kable_styling(latex_options = "hold_position")
```

For the first analysis we focus our attention on activity 38 and the time series \text{bpm}, plotted in figure \ref{fig:plot_bpm}: 
...
```{r, fig.width=6.5, fig.height=3, fig.cap="\\label{fig:plot_bpm} Recorded bpm activity 38"}

# Time series plot
ggplot(track_38, aes(x=time, y=bpm)) +
  geom_line(color="steelblue") +
  theme_bw(base_size=9) +
  labs(y="Bpm", x="Time(sec)") +
...
It is possible to see that the time series does not look stationary and that several change points characterize the plot, e.g. at 4600sec and 5400sec. Therefore, it is not possible to analyse this series with the well-known ARMA models, which require stationarity (or non-stationary time series that can be easilty trasformed into stationary ones). A suitable class of model for these cases is given by the Hidden Markov Models. We consider a latent process $(S_{t})_{t\geq0}$ which  represents the required athletic effort: low effort, medium effort and high effort. We assume tha the process $(S_{t})_{t\geq0}$ is a \textit{homogeneous} Markov chain, with state space $\{1, 2, 3\}$

\textbf{Step 1: Model specification}
We assume a Gaussian emission distribution, with state-dependent mean $\mu_i$ and variance $\sigma_i$ with $i=1,2,3$: 
...
```{r}
# Summary table creation
MLEsum_bpm <- data.frame(state=c(rep("S=1",2), rep("S=2",2), rep("S=3",2)), mu=c(mu1hat_bpm, se_mu1hat_bpm, mu2hat_bpm, se_mu2hat_bpm, mu3hat_bpm, se_mu3hat_bpm), sd=c(sd1hat_bpm, se_sd1hat_bpm, sd2hat_bpm, se_sd2hat_bpm, sd3hat_bpm, se_sd3hat_bpm))
knitr::kable(MLEsum_bpm, "latex", booktabs=T, align="c", col.names = c("Hidden state", "Mean", "Standard deviation"), escape=F, caption="MLE estimators of HMM\\label{tab:HMM_MLE_bpm}") %>% 
...
\textbf{Step 3: Decoding}
Finally, we find the "oprimal" state sequence associated with the observed \textit{bpm} time series. The optimization problem is solve using the \textit{Viterbi algorithm} and the results are shown in figure \ref{fig:model_fit_ost_bpm}: 
