...
density1<-curve(post_alpha_ibm, from=-0.05, to=0.05, xlab="alpha", ylab="posterior density")

post_beta_ibm = function(beta){dnorm(beta, outFilter_ibm$m[length(y_1)+1,2], CT_ibm[2,2]^.5)}
density2<-curve(post_beta_ibm, xlab="beta", ylab="posterior density")
```


```{r, fig.height=5, fig.width=15, fig.cap= "Posterior densities of model's parameters"}
# Plots
density_plot<-data.frame(density1,density2)

D1 <- ggplot(density_plot, aes(x=x, y=y)) +
  geom_line(size=1) +
...
D2 <- ggplot(density_plot, aes(x=x.1, y=y.1)) +
  geom_line(size=1) +
  theme_stata(base_size=9) +
  labs(y="", x="beta") +
...
density1<-curve(post_alpha_citi, from=-0.05, to=0.05, xlab="alpha", ylab="posterior density")

post_beta_citi = function(beta){dnorm(beta, outFilter_citi$m[length(y_2)+1,2], CT_citi[2,2]^.5)}
density2<-curve(post_beta_citi, xlab="beta", ylab="posterior density")
```
