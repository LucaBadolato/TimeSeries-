...
```{r, fig.width=20, fig.height=6, out.width='100%', fig.cap="\\label{fig:stock_plots} Stock plots"}
# Plot
...
\end{eqnarray*}
In particular, we set $G=I_2$ and $W=diag(\sigma^2_{w_1}, \sigma^2_{w_2})$, that is $(\alpha_t)_{t \geq 0}$ and $(\beta_t)_{t \geq 0}$ are independent random walks. In addition, in order to have static regression coefficients, we set $\sigma^2_{w_1}=0$ and $\sigma^2_{w_2}=0$.
...
Finally, the filtering distributions of $\alpha \mid y_{1:T}$ and $\beta \mid y_{1:T}$ are plotted in the Figure \ref{fig:post_est}.
...
\theta_t=[\alpha_t, \beta_t]', \qquad  \theta_0 \sim N_2(m_0, C_0), \qquad \theta_0 \bot (v_t) \bot (w_t)
\end{eqnarray*}
and $G=I_2$ and $W=diag(0, 0)$. Furthermore, given the information we obtained in the previous exercise, we follow a proper Bayesian approach considering as initial expectation the vector $m_{0} = (0, 1.5)$, that is taking $E(\alpha)=0$ and $E(\beta)=0.45$, with variance matrix $C_0 = diag(0.0002, 0.005)$. Indeed, even if we are considering a different asset, we can think that we can borrow some information. Notice that we slightly increase the variances with respect with the posterior estimates obtained in the previous section to account for the uncertainty we have in this context.
...
Then, recalling that the posterior distribution of $\alpha \mid y_{1:T}$ is $N(m_T, C_T)$, we plot the filtering distributions of $\alpha \mid y_{1:T}$ and $\beta \mid y_{1:T}$ in figure \ref{fig:post_est2} (blue lines). Furthermore, we plotted also the prior distributions of the model's parameters (red lines). 
