...
This model assumes that the *excess returns*, $Y_t$, of a risky financial asset depend linearly on *market excess returns*, $x_t$, or, stated differently, it assumes that the only relevant source of risk for pricing purposes is the systematic risk, namely the one that cannot be diversified away. 
...
\item $\alpha$ is not significantly different from zero under the model. Hence, a $\hat{\alpha}$ significantly different from zero is regarded as *pricing error* (or *Jensen's $\alpha$*).
\end{itemize}
...
If we were to estimate with Maximum likelihood the parameters $\alpha$ and $\beta$, they would coincide with the $OLS$ estimates.
