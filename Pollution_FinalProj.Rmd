...
Iniziamo con modelli semplici e poi mano a mano li complichiamo per renderli più adeguati alla nostra serie. Inizio ad adattare il random walk plus noise dell'esercizio 5: per ora ho messo i parametri "a caso", poi provo a utilizzare gli MLE presentati nell'ultimo lab. \bigskip
...
\end{eqnarray*}
where the variances of both processes are constant and set at $V=\sigma^2_v=500$ and $W=\sigma^2_w=100$ respectively.

The prior distribution is also assumed to be Gaussian with parameters $m_0=0$ and $C_0=1$, that is:
\begin{align*} 
\theta_0 \sim N(0.75, 10)
\end{align*}
...
```{r}
# Polinomial specification of the RW + noise
rw_CO <- dlmModPoly(order=1,dV=0.11, dW=0.25, m0=0.75, C0=0.1)
...
# Df
df_CO_filt <-data.frame(louisiana_df$n_weeks, louisiana_df$weekly_CO_mean, filt_est)
colnames(df_CO_filt) <- c("week", "CO", "filt_est")
# Plot
