...
ADF_results<-matrix(NA,2,4)

for (col_idx in 1:4){
  ADF<-adf.test(louisiana_df[,col_idx+1]) 
  ADF_results[1,col_idx]<-ADF$statistic
  ADF_results[2,col_idx]<-ADF$p.value
}

# results
knitr::kable(ADF_results[,1:4], "latex", booktabs=T, align=c(rep('c',times=4)),
             col.names=(c("CO","NO2","O3","SO2")),
             caption="Augmented Dickey-Fuller test\\label{tab:ADF_test}") %>%
  kable_styling(position = "center", latex_options="hold_position")

```

A formal test for presence of unit roots against the alternative of stationarity is also carried out to confirm our guess. Results for the Augmented Dickey-Fuller test are reported in table \ref{tab:ADF_test}.
...
\section{HMM for modelling trend}
...
that is, in fact, a \textit{non-homogeneous} HMM, where the covariate which we allow to correlate with the observation process is simply the time. Indeed, a common way to construct models belonging to this class is to let $Y_t$ be distributed as $N(\alpha_i + x\beta_i, \; \sigma_j^2)$, where the location parameter is specified as a state-dependent linear regression.
...
# Kalman smoother
smooth_CO <- dlmSmooth(filt_CO)

str(smooth_CO, 1)
attach(smooth_CO)
# Remove the first item (m_0) in the vector of moothed states
smooth_est <- dropFirst(smooth_CO$s)
vol_S_smooth <-sqrt(unlist(dlmSvd2var(smooth_CO$U.S, smooth_CO$D.S)))
...
GRAFICI INTERESSANTI CHE POSSIAMO UTILIZZARE PER PRESENTARE IN MANIERA SCHEMATICA E RIASSUNTIVA I RISULTATI: (PAG.92 E 93 )

```{r, fig.width=7, fig.height=2, fig.cap="\\label{fig:prova} Figure at page 92"}
...
vol_S_plot <- vol_C[2:853] 
upper_smooth <- smooth_est + 1.96*vol_S_plot 
lower_smooth <- smooth_est - 1.96*vol_S_plot

df_CO_ci_smooth <-data.frame(louisiana_df$n_weeks, louisiana_df$weekly_CO_mean, smooth_est, upper_smooth, lower_smooth)
colnames(df_CO_ci_smooth) <- c("week", "CO", "smooth_est", "upper", "lower")  
colors <- c("Observed data" = "black", "Smoothing estimates with C.I." = "red")

 C <- ggplot(df_CO_ci_smooth, aes(x=week))+
  geom_line(aes(y=CO, color = "Observed data"), size=0.3) +
  geom_line(aes(y=smooth_est, color = "Smoothing estimates with C.I."), size=0.5) +
  geom_line(aes(y=upper_smooth), color = "red", linetype = "dashed", size=0.1) +
  geom_line(aes(y=lower_smooth), color = "red", linetype = "dashed", size=0.1) +
  geom_ribbon(aes(ymin=lower_smooth, ymax=upper_smooth), alpha=.1, fill="red") +
  theme_classic(base_size=9.5) +
  theme(panel.grid.minor = element_line(size=0.5)) +
  labs(y=expression(CO~(ppm)), x="Week", colors = "") +
  scale_x_continuous(breaks=seq(0,900,100)) +
  theme(axis.title.x=element_text(family="Times", margin=margin(t=5)),
        axis.title.y=element_text(family="Times", margin=margin(r=5)))+
   scale_color_manual(values = colors)+
...
df_CO_sd_smooth <-data.frame(louisiana_df$n_weeks, vol_S_plot)
colnames(df_CO_sd_smooth) <- c("week", "vol_S_plot")

D <- ggplot(df_CO_sd_smooth, aes(x=week))+
  geom_line(aes(y=vol_S_plot), color = "darkgreen", size=0.4) +
  theme_classic(base_size=9.5) +
  theme(panel.grid.minor = element_line(size=0.5)) +
  labs(x="Year", y="Standard deviation") +
  labs(y="Standard deviation", x="Week", color = "") +
