...
```{r, fig.width=5, fig.height=2.5, fig.cap="\\label{fig:Nile_plot}Annual flow of the river Nile at Ashwan"}
# Df
...
Let's indeed model the series with a random walk plus noise (or local level) model, a time-invariant DLM with univariate Gaussian observation and state process, i.e. $\underset{1 \times 1}{Y_t}$ and $\underset{1 \times 1}{\theta_t}$, and with constant system matrices $\underset{1 \times 1}F=1$ and $\underset{1 \times 1}G=1$:
\begin{eqnarray*} 
...
Our first goal is to compute the \textit{filtering states estimates} $m_t =\mathbb{E}\;(\theta_t \mid y_{1:t})$ obtained by implementing the Kalman filter. In particular, we recall that the Kalman filter procedure, given the \textit{filtering distribution} in the previous iteration, $\theta_{t-1} \mid \sim N(m_{t-1},C_{t-1})$, follows three steps:
...
```{r, fig.width=5, fig.height=2, fig.cap="\\label{fig:filter_est} Filtering states estimates"}
# Df
df_Nile_filt <-data.frame(years_Nile, Nile, filt_est)

# Plot
ggplot(df_Nile_filt, aes(x=years_Nile)) +
  geom_line(aes(y=Nile), color="black", size=0.4) +
  geom_line(aes(y=filt_est), color = "red", size=0.4) +
  geom_vline(xintercept = as.Date('1899/01/01'), color= "black", size=0.3) +
  theme_classic(base_size=9.5) +
...
```{r, fig.width=5, fig.height=2.5, fig.cap="\\label{fig:filter_est_ci} Filtering estimates with credible intervals"}
...
df_Nile_ci <-data.frame(years_Nile, Nile, filt_est, upper, lower)
 ...
 ggplot(df_Nile_sd, aes(x=years_Nile))+
  geom_line(aes(y=Nile), color = "black", size=0.3) +
  geom_line(aes(y=filt_est), color = "red", size=0.5) +
  geom_line(aes(y=upper), color = "red", linetype = "dashed", size=0.3) +
  geom_line(aes(y=lower), color = "red", linetype = "dashed", size=0.3) +
...
```{r, fig.width=5, fig.height=2.5, fig.cap="\\label{fig:forecast_ci} One-step ahead forecasts with credible intervals"}
# Df
...
ggplot(df_Nile_for, aes(x=years_Nile)) +
  geom_line(aes(y=Nile), color = "black", size=0.3) +
  geom_line(aes(y=mean_forecast), color = "blue", size=0.5) +
  geom_line(aes(y=upper_for), color = "blue", linetype = "dashed", size=0.3) +
  geom_line(aes(y=lower_for), color = "blue", linetype = "dashed", size=0.3) +
...
The behavior of the process $(Y_t)$ is highly influenced by the ratio between the evolution and the observations error variances, $r=\frac{W}{V}=\frac{\sigma^2_w}{\sigma^2_v}$, that is the so-called \textit{signal-to-noise ratio}.

For any given $C_0$, if the \textit{signal-to-noise ratio} is small, $K_t$ is small and as a consequence $y_t$ receives little weight. On the contrary, if the ratio is large, $y_t$ receives large weight and the one-step-ahead forecast is close to the most recent data point.

We repeat the same analysis of Questions 1 and 2 choosing two different values for the abovementioned process variances in order to find evidence that this is actually the case.
\bigskip
...
\end{eqnarray*}
where, differently from the previous exercise, we have set the process variances equal to $V=\sigma^2_v=2000$ and $W=\sigma^2_w=8000$.
The \textit{signal-to-noise ratio} is thus $r=\frac{W}{V}=\frac{8000}{2000}=4$.
...
```{r, fig.width=5, fig.height=2.5, fig.cap="\\label{fig:filter_est_ci_2} Filtering estimates with credible intervals"}
...
df_Nile_ci_2 <-data.frame(years_Nile, Nile, filt_est_2, upper_2, lower_2)
 ...
 ggplot(df_Nile_sd_2, aes(x=years_Nile))+
  geom_line(aes(y=Nile), color = "black", size=0.3) +
  geom_line(aes(y=filt_est_2), color = "red", size=0.5) +
  geom_line(aes(y=upper_2), color = "red", linetype = "dashed", size=0.3) +
  geom_line(aes(y=lower_2), color = "red", linetype = "dashed", size=0.3) +
...
```{r, fig.width=5, fig.height=2.5, fig.cap="\\label{fig:forecast_ci_2} One-step ahead forecasts with credible intervals"}
# Df
...
ggplot(df_nile_for_2, aes(x=years_Nile)) +
  geom_line(aes(y=Nile), color = "black", size=0.3) +
  geom_line(aes(y=mean_forecast_2), color = "blue", size=0.5) +
  geom_line(aes(y=upper_for_2), color = "blue", linetype = "dashed", size=0.3) +
  geom_line(aes(y=lower_for_2), color = "blue", linetype = "dashed", size=0.3) +
