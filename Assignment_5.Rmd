...
We now plot along with the data the one-step ahead forecasts for the observation process, i.e. $\hat{y}_{t}=\E{Y_{t} | y_{1:t-1}}=f_t$, where $f_t$ is the location parameter of the observation's forecast distribution. We plot also the credible interval at $1-\alpha$ level that are given by:
...
upper <- mean_forecast + 1.96*sqrt(filt_Q) 
lower <- mean_forecast - 1.96*sqrt(filt_Q) 
