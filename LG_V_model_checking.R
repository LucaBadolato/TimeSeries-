...
ggplot(res_LG_v, aes(x=time)) +
  ...
  theme(axis.text.x=element_text(size=10, family="Times", margin=margin(t=5)),
        axis.text.y=element_text(size=10, family="Times", margin=margin(r=5)),
...
b <- ggplot.corr(data=res_LG_v$res, lag.max=12, ci=0.95, large.sample.size=FALSE, horizontal=TRUE)

c <- ggplot(res_LG_v, aes(sample=res)) +
